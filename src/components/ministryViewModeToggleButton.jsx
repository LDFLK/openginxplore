...
export default function MinistryViewModeToggleButton({ viewMode, setViewMode }) {
  ...
  return (
    <div className="flex justify-end w-full sm:w-auto">
      <div className="relative flex bg-transparent border-2 border-foreground/20 rounded-md w-38 h-10 select-none">
        <div
        ...
        >
          <MdGridOn className={`text-base transition-all ${!isGraph ? "opacity-100" : "opacity-50"}`} />
          List
        </button>
        ...
        >
          <PiGraph className={`text-base transition-all ${isGraph ? "opacity-100" : "opacity-50"}`} />
          Graph
        </button>
      </div>
    </div>
